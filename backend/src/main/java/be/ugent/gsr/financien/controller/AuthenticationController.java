...
public class AuthenticationController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<JwtAuthenticationResponse> signup(@RequestBody RegistreerDAO request) {
        return ResponseEntity.ok(authenticationService.registreer(request));
    }

    @PostMapping("/signin")
    public ResponseEntity<JwtAuthenticationResponse> signin(@RequestBody LoginDAO request) {
