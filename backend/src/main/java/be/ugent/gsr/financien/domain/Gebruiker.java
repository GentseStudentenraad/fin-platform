...
public class Gebruiker implements UserDetails {
    ...
    @ManyToMany
    @JoinTable(
            name = "user_organisatie",
            joinColumns = @JoinColumn(name = "gebruiker_id"),
            inverseJoinColumns = @JoinColumn(name = "organisatie_id")
    )
    ...
    public List<SubBudgetPost> visibleBudgetPosts(SubBudgetPostRepository subBudgetPostRepository, Integer jaar) {
        if (isVoorzitterOrBeheerder()) {
            return subBudgetPostRepository.findAll().stream().filter(
                            //Filter so that only the subBudgetPosts of the given year get added
                            subBudgetPost -> Objects.equals(subBudgetPost.getBudgetPost().getBoekjaar().getJaartal(), jaar))
                    .toList();
        } else {
            // All the subBudgetPosts based on the organisations
            ArrayList<SubBudgetPost> result = new ArrayList<>(this.organisatie.stream().flatMap(org -> org.getRol().stream().flatMap(rol ->
                    rol.getSubBudgetPost().stream().filter(
                            //Filter so that only the subBudgetPosts of the given year get added
                            subBudgetPost -> Objects.equals(subBudgetPost.getBudgetPost().getBoekjaar().getJaartal(), jaar)
                    ))).toList());
            // Add all the subBudgetPosts based on the roles of this user
            result.addAll(this.getRol().stream().flatMap(rol -> rol.getSubBudgetPost().stream().filter(
                    //Filter so that only the subBudgetPosts of the given year get added
                    subBudgetPost -> Objects.equals(subBudgetPost.getBudgetPost().getBoekjaar().getJaartal(), jaar))
            ).toList());
            return result;
